...
> 基于 Air8000 模块，为 Safex 便携式化学毒气检测仪提供 WiFi 通信、定位、蓝牙扫描、跌倒检测及 MQTT 数据上报功能。

## 项目概述

本系统运行在 Air8000 模块上，采用分层架构，各功能模块独立运行，通过数据中心（`app_data`）统一管理数据。支持 WiFi AP/STA 双模式联网，提供 MQTT 远程通信和 OTA 固件升级功能。
...
所有功能可通过屏幕命令码或 MQTT 控制开关，关闭后对应数据不上报：
...
| `mqtt_server` | `"192.168.5.98"` | MQTT 服务器地址（空则不连接） |
...
| 主题 | 方向 | 说明 |
|------|------|------|
| `chem/telemetry/<device_id>` | 设备→服务器 | 定时遥测上报（传感器/GNSS/系统状态） |
| `chem/events/<device_id>` | 设备→服务器 | 报警事件上报（触发/清除，即时推送） |
| `chem/notify` | 服务器→设备 | 群发通知（所有设备监听） |
| `chem/<device_id>/notify` | 服务器→设备 | 单发通知（仅指定设备监听） |
...
printh 70 23 FF FF FF    MQTT 连接（MCU 拉取 t3/t4）
printh 70 24 FF FF FF    MQTT 断开
printh 70 25 FF FF FF    设置上报间隔（MCU 拉取 t8）

MQTT 设置（TJC prints 指令，配合 printh 70 + printh FF FF FF）：
prints "SET_MQTT_SERVER 192.168.5.98"
...
│   ├── mod_mqtt.lua             # MQTT 通信（WiFi 联网 + MQTT 收发 + 报警事件上报）
