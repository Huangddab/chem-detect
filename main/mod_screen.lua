...
-- 地图偏移拉取状态（MCU 主动从屏幕 t5/t6 读取）
local map_pull_step   = 0         -- 0=空闲 1=等待X偏移 2=等待Y偏移
local map_pull_x      = nil       -- 临时存储拉取到的 X 偏移
local map_pull_timer  = nil       -- 拉取超时定时器

-- 地图设备测试拉取状态（MCU 主动从屏幕 t7/t8 读取纬度/经度, 临时测试功能）
local map_dev_pull_step  = 0      -- 0=空闲 1=等待纬度 2=等待经度
local map_dev_pull_lat   = nil    -- 临时存储拉取到的纬度
local map_dev_pull_timer = nil    -- 拉取超时定时器

-- 前向声明 (handle_map_pull_response 在 handle_sta_pull_response 中被调用, 需提前声明)
local handle_map_pull_response
-- 前向声明 (handle_map_dev_pull_response 在 handle_sta_pull_response 中被调用, 需提前声明)
local handle_map_dev_pull_response
...
local function handle_sta_pull_response(str)
    ...
    elseif sta_pull_step == 2 then
        ...
        -- 检查是否是地图偏移拉取响应
        if map_pull_step ~= 0 then
            handle_map_pull_response(str)
            return
        end
        -- 检查是否是地图设备测试拉取响应
        if map_dev_pull_step ~= 0 then
            handle_map_dev_pull_response(str)
            return
        end
        -- 检查是否是 MQTT 拉取响应
...
-- 地图偏移拉取：MCU 主动从屏幕 t5/t6 读取 X/Y 偏移
-- 屏幕端发送 printh 70 26 FF FF FF 触发
local function handle_map_pull()
    if map_pull_step ~= 0 then
        log.warn("SCREEN", "地图偏移拉取正在进行中, 忽略重复触发")
        return
    end
    log.info("SCREEN", "地图偏移拉取: 请求 t5.txt (X偏移)")
    map_pull_step = 1
    map_pull_x = nil
    sys.timerStart(function()
        mod_screen.get_text("t5")
    end, 50)
    map_pull_timer = sys.timerStart(function()
        if map_pull_step ~= 0 then
            log.error("SCREEN", "地图偏移拉取 X 超时")
            map_pull_step = 0
            map_pull_x = nil
        end
        map_pull_timer = nil
...
-- 地图偏移拉取响应处理
handle_map_pull_response = function(str)
    if map_pull_step == 1 then
        -- 收到 X 偏移，继续拉取 Y
        map_pull_x = str
        log.info("SCREEN", "地图偏移拉取 X:", str)
        map_pull_step = 2
        if map_pull_timer then sys.timerStop(map_pull_timer) end
        sys.timerStart(function()
            mod_screen.get_text("t6")
        end, 50)
        map_pull_timer = sys.timerStart(function()
            if map_pull_step ~= 0 then
                log.error("SCREEN", "地图偏移拉取 Y 超时")
                map_pull_step = 0
                map_pull_x = nil
            end
            map_pull_timer = nil
        end, 2000)
    elseif map_pull_step == 2 then
        -- 收到 Y 偏移，应用偏移
        local y_str = str
        local x_str = map_pull_x or "0"
        local x = tonumber(x_str) or 0
        local y = tonumber(y_str) or 0
        log.info("SCREEN", string.format("地图偏移拉取完成: x=%s, y=%s", x_str, y_str))
        map_pull_step = 0
        map_pull_x = nil
        if map_pull_timer then sys.timerStop(map_pull_timer); map_pull_timer = nil end
        -- 通过事件通知地图模块更新偏移
        sys.publish("MAP_TILE_OFFSET_CHANGED", x, y)
    end
end

-- 地图设备测试拉取：MCU 主动从屏幕 t7/t8 读取纬度/经度
-- 屏幕端发送 printh 70 27 FF FF FF 触发
-- 读取 t7=纬度, t8=经度, 作为测试设备坐标刷新 p1 标记
local function handle_map_dev_pull()
    if map_dev_pull_step ~= 0 then
        log.warn("SCREEN", "地图设备测试拉取正在进行中, 忽略重复触发")
        return
    end
    log.info("SCREEN", "地图设备测试拉取: 请求 t7.txt (纬度)")
    map_dev_pull_step = 1
    map_dev_pull_lat = nil
    sys.timerStart(function()
        mod_screen.get_text("t7")
    end, 50)
    map_dev_pull_timer = sys.timerStart(function()
        if map_dev_pull_step ~= 0 then
            log.error("SCREEN", "地图设备测试拉取 纬度 超时")
            map_dev_pull_step = 0
            map_dev_pull_lat = nil
        end
        map_dev_pull_timer = nil
    end, 2000)
end

-- 地图设备测试拉取响应处理
handle_map_dev_pull_response = function(str)
    if map_dev_pull_step == 1 then
        -- 收到纬度，继续拉取经度
        map_dev_pull_lat = str
        log.info("SCREEN", "地图设备测试拉取 纬度:", str)
        map_dev_pull_step = 2
        if map_dev_pull_timer then sys.timerStop(map_dev_pull_timer) end
        sys.timerStart(function()
            mod_screen.get_text("t8")
        end, 50)
        map_dev_pull_timer = sys.timerStart(function()
            if map_dev_pull_step ~= 0 then
                log.error("SCREEN", "地图设备测试拉取 经度 超时")
                map_dev_pull_step = 0
                map_dev_pull_lat = nil
            end
            map_dev_pull_timer = nil
        end, 2000)
    elseif map_dev_pull_step == 2 then
        -- 收到经度，更新设备坐标
        local lng_str = str
        local lat_str = map_dev_pull_lat or "0"
        local lat = tonumber(lat_str) or 0
        local lng = tonumber(lng_str) or 0
        log.info("SCREEN", string.format("地图设备测试拉取完成: lat=%s, lng=%s", lat_str, lng_str))
        map_dev_pull_step = 0
        map_dev_pull_lat = nil
        if map_dev_pull_timer then sys.timerStop(map_dev_pull_timer); map_dev_pull_timer = nil end
        -- 写入 app_data 并触发地图标记刷新
        app_data.update_map_device("test_device", lat, lng)
    end
end
...
--     0x0C  IMS_SOURCE_ON  — 进入寻源模式 (持续刷新 s0 双波峰图)
--     0x0E  IMS_SOURCE_OFF — 退出寻源模式 (停止自动刷新)
...
--     0x15  MAP_ON         — 地图开启
--     0x16  MAP_OFF        — 地图关闭
--     0x1B  MAP_WGS84      — 地图坐标系 WGS84 (不纠偏)
--     0x1C  MAP_GCJ02      — 地图坐标系 GCJ02 (高德/腾讯, 默认)
--     0x1D  MAP_BD09        — 地图坐标系 BD09 (百度)
--     0x22  BUZZER_TOGGLE  — 蜂鸣器直接响/停切换 (GPIO 测试)
--     0x23  MQTT_CONNECT    — MQTT 连接
--     0x24  MQTT_DISCONNECT — MQTT 断开
--     0x25  MQTT_SET_INT    — 设置上报间隔
--     0x26  MAP_TILE_OFFSET — 地图瓦片偏移拉取 (MCU 从 t5/t6 读取)
--     0x27  MAP_DEV_TEST    — 地图设备测试拉取 (MCU 从 t7/t8 读取, 临时测试)
--
...
local REQUEST_HANDLERS = {
    ...
    -- IMS 命令 (0x0C = 进入寻源模式, 0x0E = 退出寻源模式)
    [string.char(0x0C)] = function() sys.publish("IMS_SOURCE_MODE", true) end,
    ...
    -- 退出寻源模式 (0x0E)
    [string.char(0x0E)] = function() sys.publish("IMS_SOURCE_MODE", false) end,
    ...
    -- 地图坐标系切换 (0x1B~0x1D, 二进制无参数)
    [string.char(0x1B)] = function()
        app_data.set_config("map_coord_system", "wgs84")
        sys.publish("MAP_OFFSET_CHANGED")
        log.info("SCREEN", "地图坐标系: WGS84 (不纠偏)")
    end,
    [string.char(0x1C)] = function()
        app_data.set_config("map_coord_system", "gcj02")
        sys.publish("MAP_OFFSET_CHANGED")
        log.info("SCREEN", "地图坐标系: GCJ02 (高德/腾讯)")
    end,
    [string.char(0x1D)] = function()
        app_data.set_config("map_coord_system", "bd09")
        sys.publish("MAP_OFFSET_CHANGED")
        log.info("SCREEN", "地图坐标系: BD09 (百度)")
    end,
    ...
    -- MQTT DIRECT SET 命令 (0x23=连接 b1, 0x24=断开 b2, 0x25=设置上报间隔)
    [string.char(0x23)] = function() sys.publish("MQTT_CONNECT") end,
    [string.char(0x24)] = function() sys.publish("MQTT_DISCONNECT") end,
    [string.char(0x25)] = function() sys.publish("MQTT_SET_INTERVAL") end,
    -- 地图偏移拉取 (0x26, MCU 主动从屏幕 t5/t6 读取 X/Y 偏移)
    [string.char(0x26)] = function()
        handle_map_pull()
    end,
    -- 地图设备测试拉取 (0x27, MCU 主动从屏幕 t7/t8 读取纬度/经度, 临时测试)
    [string.char(0x27)] = function()
        handle_map_dev_pull()
    end,
    ...
    -- 地图坐标系切换也支持 ASCII 指令 (兼容 UART1)
    ["SET_MAP_OFFSET"] = function(param)
        if not param or param == "" then
            log.warn("SCREEN", "SET_MAP_OFFSET 缺少参数, 可选: wgs84/gcj02/bd09")
            return
        end
        local coord = param:lower():match("^%s*(%a+)%s*$")
        if coord ~= "wgs84" and coord ~= "gcj02" and coord ~= "bd09" then
            log.warn("SCREEN", "SET_MAP_OFFSET 未知坐标系: " .. param .. ", 可选: wgs84/gcj02/bd09")
            return
        end
        app_data.set_config("map_coord_system", coord)
        sys.publish("MAP_OFFSET_CHANGED")
        log.info("SCREEN", "地图坐标系切换: " .. coord:upper())
    end,
}
        ...
        if #rx_buffer > 0 then
            ...
            rx_timer_id = sys.timerStart(function()
                if #rx_buffer > 0 then
                    ...
                    local mod_screen_mqtt = require "mod_screen_mqtt"
                    if (sta_pull_step ~= 0 or map_pull_step ~= 0 or map_dev_pull_step ~= 0 or mod_screen_mqtt.get_pull_step() ~= 0) and string.byte(rx_buffer, 1) == 0x70 then
