...
-- 间隔拉取状态（MCU 主动从屏幕 t8 读取上报间隔）
local interval_pull_step  = 0      -- 0=空闲 1=等待间隔值
local interval_pull_timer = nil    -- 拉取超时定时器
...
local function get_text(widget)
    if screen_api and screen_api.get_text then
        log.debug("SCR_MQTT", "get_text:", widget)
        screen_api.get_text(widget)
    else
        log.error("SCR_MQTT", "screen_api.get_text 不可用!", "screen_api=", tostring(screen_api))
    end
end
...
-- t3=服务器地址  t4=端口  t5=设备ID  t6=日志/状态  t8=上报间隔
local function handle_get_mqtt()
    ...
    set_text(WIDGET.interval, tostring(interval))
...
function mod_screen_mqtt.handle_pull_response(str)
    log.info("SCR_MQTT", "handle_pull_response: mqtt_step=", mqtt_pull_step, "interval_step=", interval_pull_step, "str=", tostring(str), "hex=", str and str:toHex() or "nil", "len=", str and #str or 0)

    -- 间隔拉取响应
    if interval_pull_step == 1 then
        local interval_str = str
        log.info("SCR_MQTT", "拉取上报间隔:", interval_str)
        interval_pull_step = 0
        if interval_pull_timer then sys.timerStop(interval_pull_timer); interval_pull_timer = nil end

        local interval = tonumber(interval_str) or 0
        if interval <= 0 then
            log.warn("SCR_MQTT", "上报间隔无效:", interval_str)
            set_text(WIDGET.log, "Invalid interval")
            return
        end
        app_data.set_config("report_interval", interval)
        log.info("SCR_MQTT", "上报间隔已设置:", interval, "s")
        set_text(WIDGET.log, "Interval: " .. interval .. "s")
        return
    end
    ...
    if mqtt_pull_step == 1 then
        ...
        log.info("SCR_MQTT", "拉取服务器地址:", str, "hex:", str:toHex())
        mqtt_pull_step = 2
    ...
    elseif mqtt_pull_step == 2 then
        ...
        log.info("SCR_MQTT", "拉取端口:", port_str, "hex:", port_str:toHex())
        mqtt_pull_step = 0
        mqtt_pull_server = nil
        ...
        -- 通知 mod_mqtt 断开旧连接并重连新地址
        local mqtt_api = app_data.get_callback("mqtt_api")
        if mqtt_api and mqtt_api.reconnect then
            mqtt_api.reconnect()
        else
            log.error("SCR_MQTT", "mqtt_api 回调未注册")
        end
        log.info("SCR_MQTT", "MQTT 连接:", server, port)
    end
end

-- 设置上报间隔按钮 (0x25): MCU 主动从屏幕 t8 拉取上报间隔
function mod_screen_mqtt.handle_set_interval()
    if interval_pull_step ~= 0 then
        log.warn("SCR_MQTT", "间隔拉取正在进行中, 忽略重复触发")
        return
    end
    log.info("SCR_MQTT", "拉取上报间隔: 请求 t8.txt")
    interval_pull_step = 1
    sys.timerStart(function()
        get_text(WIDGET.interval)
    end, 50)
    interval_pull_timer = sys.timerStart(function()
        if interval_pull_step ~= 0 then
            log.error("SCR_MQTT", "拉取上报间隔超时")
            interval_pull_step = 0
            set_text(WIDGET.log, "Pull interval timeout")
        end
        interval_pull_timer = nil
    end, 2000)
end
...
function mod_screen_mqtt.start()
    ...
    -- 订阅设置上报间隔事件 (由 mod_screen 0x25 命令码发布)
    sys.subscribe("MQTT_SET_INTERVAL", function()
        sys.taskInit(function()
            mod_screen_mqtt.handle_set_interval()
        end)
    end)

    -- 订阅 MQTT 连接状态事件 (由 mod_mqtt 发布)
    sys.subscribe("MQTT_STATUS", function(status)
        sys.taskInit(function()
            local msg_map = {
                online         = "MQTT ONLINE",
                disconnected   = "MQTT LOST",
                timeout        = "MQTT TIMEOUT",
                handshake_fail = "MQTT AUTH FAIL",
                tcp_fail       = "MQTT TCP FAIL",
                create_fail    = "MQTT CREATE FAIL",
            }
            local msg = msg_map[status] or ("MQTT " .. tostring(status))
            set_text(WIDGET.log, msg)
            log.info("SCR_MQTT", "MQTT 状态:", status, "->", msg)
        end)
    end)

    -- 订阅配置变更事件 (仅当非 MQTT 专用配置变更时刷新屏幕)
...
function mod_screen_mqtt.get_pull_step()
    if mqtt_pull_step ~= 0 then return mqtt_pull_step end
    if interval_pull_step ~= 0 then return interval_pull_step end
    return 0
end
