...
local wlan_inited = false   -- wlan.init() 只调用一次
...
-- ========== 安全初始化 WiFi 芯片（只调一次） ==========
local function ensure_wlan_init()
    if not wlan_inited then
        wlan.init()
        wlan_inited = true
        sys.wait(300)
    end
...
local function start_wifi_sta()
    ...
    -- 1. 初始化 WiFi 芯片（只调一次，开机直连 STA 时官方 demo 不需要 init）
    ensure_wlan_init()

    -- 2. 设置为 STA 模式（从 AP 切回时必须，直连不需要但无副作用）
    ...
    while not got_ip do
        ...
        if wait_count % 5 == 0 then
            log.info("NET", "WiFi STA 连接中... (" .. wait_count .. "s)")
        end
        if wait_count > 50 then
            sys.unsubscribe("IP_READY", on_ip_ready)
            log.error("NET", "WiFi STA 连接超时(50秒), 请检查 SSID/密码")
            return false
        end
    end
    sys.unsubscribe("IP_READY", on_ip_ready)
...
function mod_net.start()
    sys.taskInit(function()
        ...
        -- 广播网络状态 (供 mod_mqtt 事件驱动等待, 屏幕/其他模块也订阅)
        if ok then
            sys.publish("NET_STATUS", "connected")
        else
            sys.publish("NET_STATUS", "failed")
