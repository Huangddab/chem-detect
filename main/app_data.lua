...
local data = {
    ...
    config = {
        ...
        alarm_en          = true,   -- 报警功能开关
        map_en            = true,    -- 地图显示功能开关
        map_coord_system   = "wgs84",  -- 地图坐标系: wgs84/gcj02/bd09
    },
    ...
    -- GNSS 定位数据
    -- 测试阶段: lat/lng 初始为测试坐标 (深圳宝安), 有定位时 update_gnss 覆盖
    -- 正式上线前改为 0, 由 mod_gnss 写入真实定位
    gnss = {
        lat       = 22.614118393557618,
        lng       = 113.83699825861532,
        speed     = 0,
        fixed     = false,
        timestamp = 0,
    },
    ...
    map = {
        ...
        lat        = 0,          -- 当前显示纬度 (十进制, 纠偏后)
        lng        = 0,          -- 当前显示经度 (十进制, 纠偏后)
        active       = false,      -- 地图是否已激活
        coord_system  = "gcj02",    -- 当前使用的坐标系
        timestamp    = 0,
        -- 其他设备坐标列表 (从 MQTT chem/{device_id}/notify 消息中提取的 WGS84 坐标)
        -- { {device_id="xxx", lat=22.5, lng=113.8, timestamp=123}, ... }
        devices    = {},
    },
...
function app_data.get_report_data()
    ...
    if data.config.map_en then
        report.map = {
            ...
            coord_system = data.map.coord_system,
...
-- @param detail 可选，报警详情表 (如 { names={"DMMP"}, conc=1.2 })
function app_data.update_alarm(source, detail)
    ...
    -- 广播报警事件（mod_mqtt 订阅后上报到 chem/events/{device_id}）
    sys.publish("ALARM_TRIGGER", source, data.alarm.level, detail)
end
...
function app_data.clear_alarm(source)
    ...
    data.alarm.timestamp = os.time()
    -- 广播报警清除事件
    sys.publish("ALARM_CLEAR", source)
end
...
-- 更新地图上的其他设备坐标 (从 MQTT notify 消息中收到)
-- @param device_id 设备 ID
-- @param lat        纬度 (WGS84 十进制)
-- @param lng        经度 (WGS84 十进制)
function app_data.update_map_device(device_id, lat, lng)
    if not device_id or device_id == "" then return end
    -- 查找已有设备
    local found = false
    for _, dev in ipairs(data.map.devices) do
        if dev.device_id == device_id then
            dev.lat = lat or dev.lat
            dev.lng = lng or dev.lng
            dev.timestamp = os.time()
            found = true
            break
        end
    end
    -- 新设备，添加到列表
    if not found then
        table.insert(data.map.devices, {
            device_id = device_id,
            lat       = lat or 0,
            lng       = lng or 0,
            timestamp = os.time(),
        })
        log.info("APP", "地图设备列表新增:", device_id, "共", #data.map.devices, "个设备")
    end
    -- 广播设备位置更新事件 (mod_screen_map 订阅后刷新屏幕标记)
    sys.publish("MAP_DEVICE_UPDATE", device_id)
end

-- 全量替换地图设备列表 (从 MQTT type=locations 快照消息中收到)
-- 快照语义: devices 为全量在线设备, 列表中没有的设备即下线, 直接清空重建
-- @param devices 设备数组 { {device_id="xxx", lat=22.5, lng=113.8}, ... }
-- @return 替换后的设备数量, 非法输入返回 nil
function app_data.update_map_devices(devices)
    if type(devices) ~= "table" then return nil end
    local new_list = {}
    for _, dev in ipairs(devices) do
        if type(dev) == "table" and dev.device_id and dev.device_id ~= "" then
            table.insert(new_list, {
                device_id = dev.device_id,
                lat       = tonumber(dev.lat) or 0,
                lng       = tonumber(dev.lng) or 0,
                timestamp = os.time(),
            })
        end
    end
    data.map.devices = new_list
    data.map.timestamp = os.time()
    log.info("APP", "地图设备列表全量替换:", #new_list, "个设备在线")
    -- 广播设备位置更新事件 (mod_screen_map 订阅后刷新屏幕标记)
    sys.publish("MAP_DEVICE_UPDATE")
    return #new_list
end
